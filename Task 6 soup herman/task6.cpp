...
double time_with_Soup [Experiment_size] = {0};
double time_without_soup[Experiment_size] = {0} ;
...
int main()
{
    ...
    for (int i = 0 ; i < Experiment_size ; i++) {
        ...
        time_without_soup[i] = time_with_Soup[i] = Start_cash - AT  + Cashier_time + salad_time  ;
        if (take_soup)  {
            time_with_Soup [i] += Soup_time ;
        }
