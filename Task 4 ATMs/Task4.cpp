...
const int experiment_size = 5;  // number of experiments
...
{
    random_device rd; // create a mersenne twister PRNG seeded from some implementation-defined random source
    mt19937 gen(rd());
...
int main()
{
    ...
    double total_waiting_time  = 0, Max_wait_time = 0;
    ...
    for (int i = 0 ; i < experiment_size ; i++)
    {
        ...
        {
            SST = max ( AT, Appropriate_ATM() ) ;    // start to use the next available ATM
            WT = SST - AT ;
            if (WT > Max_wait_time )
                Max_wait_time = WT ;
...
//             endl << "\t .. and used ATM : " << ATM_number+1 <<  "//////////////////\n" << endl;
        ...
        if (i == experiment_size -1)
            system_time = CT ; // system time = CT of last client
    }
